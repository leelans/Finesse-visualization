...
def _load_external_site_packages():
    """扫描系统Python环境，追加site-packages到sys.path末尾。
    优先使用打包内置的库，找不到时才fallback到外部。"""
    candidates = []
    # 1. Conda envs (active env first)
    try:
    ...
    for path_dir in os.environ.get('PATH', '').split(os.pathsep):
        for py_exe in ['python.exe', 'python3.exe']:
            ...
            if os.path.exists(full):
                try:
                    r = subprocess.run([full, '-c', 'import site; print(site.getsitepackages()[0])'],
                                      capture_output=True, text=True, timeout=5)
                    sp = r.stdout.strip()
                    if sp and os.path.isdir(sp):
                        candidates.append(sp)
                    # Also get user site-packages
                    r2 = subprocess.run([full, '-c', 'import site; print(site.getusersitepackages())'],
                                       capture_output=True, text=True, timeout=5)
                    sp2 = r2.stdout.strip()
                    if sp2 and sp2 != sp and os.path.isdir(sp2):
    ...
    # Deduplicate and append (skip incompatible Python versions)
    seen = set(sys.path)
    py_ver = f'python{sys.version_info.major}{sys.version_info.minor}'
    for sp in candidates:
        sp = os.path.normpath(sp)
        # Skip site-packages for incompatible Python versions
        if 'python3' in sp and py_ver not in sp.replace('python3.', 'python3').replace('Python3', 'Python'):
            if any(f'python{m}{n}' in sp for m in range(1,10) for n in range(0,20)
                   if f'{m}{n}' not in (str(sys.version_info.major)+str(sys.version_info.minor),)):
                continue
        if sp not in seen:
            sys.path.append(sp)
            seen.add(sp)
...
def main():
    ...
    print(f"Finesse3 Optics Simulator — Standalone")
    print(f"Server: {url}")
